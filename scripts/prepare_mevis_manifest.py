...
def prepare(meta_path: Path, split_root: Path, masks_path: Path, output: Path, max_videos: int = 20, max_items: int | None = None, expressions_per_video: int = 1) -> dict:
    ...
    for video_id in selected_videos:
        video_cases = [item for item in candidates if item[0] == video_id]
        selected.extend(video_cases if expressions_per_video <= 0 else video_cases[:expressions_per_video])
...
def main() -> None:
    ...
    parser.add_argument("--expressions-per-video", type=int, default=1, help="Expressions per selected video; use 0 for all")
    args = parser.parse_args()
    print(json.dumps(prepare(args.meta, args.split_root, args.masks, args.output, args.max_videos, args.max_items, args.expressions_per_video), indent=2))
